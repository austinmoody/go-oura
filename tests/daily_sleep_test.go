...
func TestGetSleepDocument(t *testing.T) {
	tt := []struct {
		...
		documentId     string
		mockResponse   string
		expectedOutput go_oura.SleepDocument
		expectErr      bool
	}{
		{
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			activity, err := client.GetSleepDocument(tc.documentId)
			if tc.expectErr {
				if err == nil {
...
func TestGetSleepDocuments(t *testing.T) {
	tt := []struct {
		...
		endTime        time.Time
		mockResponse   string
		expectedOutput go_oura.SleepDocuments
		expectErr      bool
	}{
		{
			...
			expectedOutput: go_oura.SleepDocuments{
				Documents: []go_oura.SleepDocument{
					{
						ID: "4eaa0e18-3464-49cc-961a-2ffd5f8ea98e",
						Day: func() go_oura.Date {
							layout := "2006-01-02"
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			activity, err := client.GetSleepDocuments(tc.startTime, tc.endTime)
			if tc.expectErr {
				if err == nil {
					t.Errorf("Expected error, got nil")
				}
